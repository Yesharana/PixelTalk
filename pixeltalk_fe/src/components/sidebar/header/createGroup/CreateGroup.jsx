...
export default function CreateGroup({setShowCreateGroup}) {
    ...
    const handleSearch = async(e)=>{
        ...
        {
            setSearchResults([]);             //Empty search result
             try {
  ...
  return (
    <div className="createGroupAnimation relative flex0030 h-full z-40">
        ...
        <div className="mt-5">
            ...
            />
            {/* Create group button */}
            <div className="absolute bottom-1/3 left-1/2 -translate-1/2">
                <button className="btn bg-green_3 scale-150 hover:bg-green-500"
                onClick={()=>createGroupHandler()}
                >
                    {
                        status==="loading" ?
                        <ClipLoader color="#E9EDEF" size={25}/> : <ValidIcon className="fill-white mt-2 h-full"/>
                    }
                </button>
            </div>
        </div>
